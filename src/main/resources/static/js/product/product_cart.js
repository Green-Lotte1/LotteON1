$(document).ready(function () {
    ...
    //******************** 장바구니 비었을때 **********************//
    //*******************************************************//
    // ${cartList}의 길이를 확인하여 비어있는지 여부를 검사합니다.
    if ($('[th:each="cart:${cartList}"]').length === 0) {
        // ${cartList}가 비어있는 경우
        $('.empty').show(); // "empty" 클래스를 가진 엘리먼트를 화면에 보이도록 설정합니다.
        $('table').hide(); // 기존의 테이블을 숨깁니다.
        $('.total').hide(); // 전체합계를 나타내는 부분도 숨깁니다.
    }
});